...
class NavigationTests extends TurboDriveTestCase {
    ...
    async "test skip link with hash-only path"() {
        await this.clickSelector("a[href=#main-content]");
        await this.nextBeat;
        const mainContent = await this.querySelector("#main-content");
        this.assert.equal(await this.remote.getActiveElement(), mainContent);
    }
}
