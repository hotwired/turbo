...
test("it prefetches immediately with a data-turbo-prefetch-delay attribute set to 'off'", async ({ page }) => {
  await goTo({ page, path: "/hover_to_prefetch.html" })

  let requestMade = false
  page.on("request", async (request) => (requestMade = true))

  await page.hover("#anchor_with_delay_off")

  assertRequestMade(requestMade)
})

test("it prefetches immediately with a turbo-prefetch-delay meta tag set to 'off'", async ({ page }) => {
  await goTo({ page, path: "/hover_to_prefetch_with_delay_off_on_meta_tag.html" })
